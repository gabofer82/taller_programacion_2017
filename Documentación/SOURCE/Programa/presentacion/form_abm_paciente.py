...
class FormPacienteABM(QtGui.QDialog, UIPacienteABM):
    ...
    def handlerBajaClicked(self):
        ...
        msgbox.setWindowTitle(u"Información")
        if not self._btn_linea_seleccionada:
            msgbox.setText(u"Atención! Este cambio es irreversible.")
            msgbox.setInformativeText(
                u"Realmente desea eliminar esta facturación?")
        else:
            msgbox.setText(u"Atención! Este cambio es irreversible.")
            msgbox.setInformativeText(
                u"{} \nRealmente desea eliminar esta linea?".format(
                    self._btn_linea_seleccionada.text()))
        msgbox.setIcon(QtGui.QMessageBox.Question)
        msgbox.addButton(u"Cancelar", QtGui.QMessageBox.RejectRole)
        msgbox.addButton(u"Eliminar", QtGui.QMessageBox.AcceptRole)

        ret = msgbox.exec_()
        # print "respuesta {}".format(ret)
        # print "acceptrole {}".format(QtGui.QMessageBox.Ok)
        if ret == 1:
            # print "largo {}".format(len(self._col_btn_lineas_facturacion))

            if len(
                    self._dict_btn_lineas_facturacion) == 1 and self._btn_linea_seleccionada:
                self._btn_linea_seleccionada.setText(u"Vacío")
                self.btn_limpiar.setEnabled(False)
                self.blanquearLineas()
                # self.actualizarTotalFactura()
            else:
                if self._btn_linea_seleccionada:
                    self._btn_linea_seleccionada.setParent(None)
                    self._btn_linea_seleccionada.deleteLater()
                    del self._dict_btn_lineas_facturacion[
                        self._btn_linea_seleccionada.nombre]
                    self._tpvadmin.quitarLinea(
                        self._btn_linea_seleccionada.data)
                    # self.actualizarTotalFactura()
                else:
                    self.eliminarLineas()

            self._btn_linea_seleccionada = None
        else:
            self.blanquearLineas()
    ...
    def handlerActualizarClicked(self):
        if self.validarFormulario():
            ...
            Acceder.nuevo_paciente(accion=u"actualizar", nombres=nombres,
                                   apellidos=apellidos,
    ...
    def cargar_campos(self, obj):
        ...
        for tel in obj.col_telefonos:
            if i == 1:
                ...
                self.agregar_telefono(tel.numero, True)
                break
            self.agregar_telefono(tel.numero, False)
            i += 1
    ...
    def agregar_telefono(self, texto, flag):
        """
